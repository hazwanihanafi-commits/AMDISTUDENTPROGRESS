...
const { readRange, writeRange } = require('../services/googleSheets');
const { driveUploadFromUrl, ensureRootFolder, createPdfReport } = require('../services/googleDrive');
...
router.get('/status', async (req, res) => {
  try {
    ...
    res.json({ status:'ok', matric, studentName: found[nameIdx], lastUpdate: found[lastIdx], milestones });
  } catch (err) { console.error(err); res.status(500).json({status:'error', message: err.toString()}); }
});
...
router.post('/approve', async (req, res) => {
  try {
    ...
    if (!matric || !stage) return res.status(400).json({status:'error', message:'missing params'});
    ...
    if (colIndex === -1) return res.status(500).json({status:'error', message:'sheet missing column ' + colName});
    const colLetter = String.fromCharCode('A'.charCodeAt(0) + colIndex);
    const range = `MasterTracking!${colLetter}${sheetRowNumber}`;
    await writeRange(SHEET_ID, range, [[ new Date().toISOString() ]]);
    await writeRange(SHEET_ID, 'Approval_Log!A1', null, { appendRow: true, values: [[new Date().toISOString(), 'approval', matric, '', stage, '', 'render-api']] });
    res.json({status:'ok', message:`Approved ${stage} for ${matric}`});
  } catch (err) { console.error(err); res.status(500).json({status:'error', message: err.toString()}); }
});

/* GET /api/approval_log -> CSV export */
...
/* JotForm webhook: POST /api/webhook */
router.post('/webhook', async (req, res) => {
  try {
    const payload = req.body;
    if (!payload.submissionStage && req.query.submissionStage) payload.submissionStage = req.query.submissionStage;
    ...
    for (const k of Object.keys(payload)) {
      const v = payload[k];
      if (!v) continue;
      const urls = String(v).split(',').map(s=>s.trim()).filter(s=>s.startsWith('http'));
      for (const u of urls) {
        try {
        ...
        } catch (err) { console.warn('file upload error', err); }
      }
    }
    const rows = await readRange(SHEET_ID, 'MasterTracking!A1:Z10000');
    const header = rows[0] || [];
    ...
    if (rIndex === -1) {
      ...
      row[matIdx] = matric;
      await writeRange(SHEET_ID, 'MasterTracking!A1', null, { appendRow: true, values: [row] });
    } else {
      const colMap = {P1:'P1 Submitted', P3:'P3 Submitted', P4:'P4 Submitted', P5:'P5 Submitted'};
    ...
    if ((payload.submissionStage || '').toUpperCase() === 'P5') {
      try {
        const folderId = await ensureRootFolder(SHEET_ID, process.env.ROOT_FOLDER_ID, matric);
        const pdf = await createPdfReport({ matric, studentName: payload.studentName42 || payload.fullname || '', files: saved, folderId });
      ...
      } catch (err) { console.warn('P5 PDF generation failed: ', err); }
    }

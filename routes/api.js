...
/* ============================================================
   GET /api/all
   Dashboard data: KPIs, stages, warnings, overduration
   ============================================================ */
router.get("/all", async (req, res) => {
  try {
    ...
    const overduration = students.filter(
      (s) => !s.p5Approved && s.timeline.status === "Overduration"
    ).length;

    const warning = students.filter(
      (s) => !s.p5Approved && s.timeline.status === "Warning"
    ).length;
...
/* ============================================================
   ...
   Used by admin, login page, student search
   ============================================================ */
router.get("/status", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      overallPercentage: student.progress.percentage,
      progressLevel: student.progress.level,
      timelineStatus: student.timeline.status,
    });
